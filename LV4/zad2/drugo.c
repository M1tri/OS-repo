...
int main(int argc, char** argv)
{
	...
	pd3[0] = atoi(argv[5]); // argv[5] == pd3[0]
	pd3[1] = atoi(argv[6]); // argv[6] == pd3[1]

	// Zatvaramo jer ne koristimo
	close(pd1[0]);
	close(pd1[1]);

	close(pd2[1]); // jer odavde cita

	close(pd3[0]); // jer ovde pise
	...
	do
	{
		...
		write(pd3[1], recenica, strlen(recenica) + 1);
